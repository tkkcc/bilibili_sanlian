...
;(async () => {
  const [position, app, _] = await waitForAll([
    '#arc_toolbar_report span.collect',
    'div#app>div.v-wrap',
    'span.bilibili-player-video-info-people-text'
  ])
  if (!position) return
  // state
  ...
  const coin_text = document.querySelector('#arc_toolbar_report span.coin > i')
    .nextSibling
    ...
    if (collect && click('#arc_toolbar_report span.collect')) {
      await new Promise(resolve => {
        new MutationObserver(function(e) {
          ...
          t =
            [...t].find(
              i => i.nextElementSibling.textContent.trim() === collection
            ) || t[0]
          ...
          const b = document.querySelector(
            'div.collection-m button.submit-move'
          )
